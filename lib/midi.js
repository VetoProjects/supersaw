...
//    var _controlMap = { };
    ...
    midi.stopLearning = function() {
        ...
        if (_learnSource.type && _learnSource.type == _learnTarget.type) {
            if (!_controlMap[_learnSource.elem] || confirm('Replace old midi binding?')) {
    ...
    var buttonClicked = function() {
        if (this.type == 'checkbox') {
            for (var command in _controlMap) {
                if (_controlMap.hasOwnProperty(command) && _controlMap[command] == this) {
    ...
    var init = function() {
        ...
        var buttons = document.querySelectorAll('button:not(#learnButton), input[type=button], input[type=checkbox]');
